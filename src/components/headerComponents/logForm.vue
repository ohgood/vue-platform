...
export default {
  ...
  methods: {
    login () {
      var self = this
      if (!self.userNameError.status || !self.passWordError.status) {
        self.errorText = '请重新检查你的输入'
      } else {
        self.errorText = ''
        if (localStorage.getItem('username') == null || localStorage.getItem('username') === '') {
          self.$http.get('http://116.196.80.176:8090/api/login?username=' + this.userName + '&password=' + this.passWord, {}, {emulateJSON: true}).then(function (response) {
          ...
          this.$emit('has-login', localStorage.getItem('username'))
        }
      }
    }
  }
